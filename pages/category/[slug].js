...
categoryItem()
  ...
  return (
    ...
    <h1 className='text-center text-[2.5rem] text-[#2a2d38] mt-[2rem]'>Category: <span className='font-bold'>{category}</span></h1>
    <p className='text-center mt-[0.5rem]'>{posts.length} articles</p>
    <p className='text-center w-[35rem] mx-auto mt-[1.5rem]'>Explore how to find your soul’s purpose and tap into the power of manifestation. 
      ...
      <div className='grid grid-cols-fluid gap-[2.5rem] mt-[3rem] mb-[3rem]'>
        ...
        return(
          <div>
            <img src={post.node.featuredImage.url} className='w-100% h-[21.5rem]'/>
            <button className='bg-black text-white p-[0.3rem] text-[0.7rem] font-semibold mt-[1.2rem]'>{post.node.categories[0].name}</button>
            <Link href={`/post/${post.node.slug}`}><h1 className='text-[1.65rem] leading-[1.8rem] font-bold text-[#2a2d38] mt-[0.8rem]'>{post.node.title}</h1></Link>
